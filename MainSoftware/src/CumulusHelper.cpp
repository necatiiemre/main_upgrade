...
bool CumulusHelper::deployNetworkInterfaces(const std::string& local_interfaces_path)
{
    ...
    // 1. Test connection
    DEBUG_LOG(getLogPrefix() << " [Step 1/3] Testing connection...");
    if (!connect())
    {
    ...
    DEBUG_LOG(getLogPrefix() << " [Step 2/3] Copying interfaces file...");
    ...
    DEBUG_LOG(getLogPrefix() << " [Step 3/3] Reloading network interfaces (ifreload -a)...");
